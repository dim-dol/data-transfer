...
def do_shutter(self):
    shutter_data = (bytes([0xa1]) + bytes([0xb2]) + bytes([dev_ID]) +
                    bytes([0xff]) + bytes([0x00]) + bytes([0x00]) + bytes([0x28]) +
                    bytes([0x00]) + bytes([0x02]) + bytes([0x2a]) + bytes([0x01]) + bytes([0x00]) +
...
class SerialController(QWidget):
    ...
    def __init__(self):
        ...
        self.shutter = QPushButton("shutter")
    ...
    def init_widget(self):
        ...
        grid_box.addWidget((self.shutter), 6, 0)
        self.shutter.clicked.connect(do_shutter)
        self.shutter.clicked.connect(lambda: self.serial.writeData(pkt))
